...
const experiences = computed(() => tm('experience.items') || [])
</script>

<template>
  <section class="section section-experience" aria-label="Experiencia profesional">
    <div class="container">
      <h2 class="section-title" v-motion="{ initial: { opacity: 0, y: 30 }, enter: { opacity: 1, y: 0, transition: { duration: 800 } } }">{{ $t('experience.title') }}</h2>

      <div class="timeline-horizontal" v-motion="{ initial: { opacity: 0 }, enter: { opacity: 1, transition: { duration: 800, delay: 200 } } }">
        ...
        <div
          ...
          @mousemove="onDrag"
        >
          <div v-for="(exp, index) in experiences" :key="index" class="timeline-item" v-motion="{ initial: { opacity: 0, y: 50 }, enter: { opacity: 1, y: 0, transition: { duration: 800, delay: 300 + (index * 100) } } }">
